...
\makeflag{prettycsm}{true}
\makeflag{twocolcsm}{true}
\makeflag{arxivcsm}{false}
\ifthenelse{\isundefined{\preprintcsm}}{}{
  \makeflag{prettycsm}{false}
  \makeflag{twocolcsm}{false}
}
\ifthenelse{\isundefined{\arxivcsm}}{}{
  \makeflag{prettycsm}{false}
  \makeflag{arxivcsm}{true}
}
...
\newif\ifPDF \ifx\pdfoutput\undefined\PDFfalse \else \ifnum\pdfoutput > 0\PDFtrue \else\PDFfalse \fi
\ifPDF
...
\colorlet{csm@sidebar@arxiv@bg}{black!2!white}
...
\colorlet{csm@table@arxiv@border}{black}
\colorlet{csm@table@bg}{csm@fig@bg}
\colorlet{csm@table@arxiv@bg}{black!4!white}
\ifmakearxivcsm
\definecolor{TitleRed}{HTML}{db6245}
\fi
...
\else
\ifmakearxivcsm
\tl_set:Nn \l_sidemargin_dim {1.2cm}
\tl_set:Nn \l_topmargin_dim {2cm}
\tl_set:Nn \l_botmargin_dim {1.4cm}
\tl_set:Nn \l_footmargin_dim {6mm}
\tl_set:Nn \l_columnsep_dim {4mm}

\xdef\csmtopmargin{\tl_use:N \l_topmargin_dim}
...
\fancypagestyle{csmfancy}{%
  ...
  \fancyhead[C]{%
    \footnotesize\sffamily\color{black!20!white}%
    Preprint%
  }%
}
\pagestyle{csmfancy}
...
% Caption style
\captionsetup[figure]{font={footnotesize,sf},labelfont={},
  name={\bfseries FIGURE~},labelsep=space}
\fi
\fi
...
\else
\gdef\csm@singlecol@emulate@width{0.7}
\newenvironment{csmfigure}[1][]{%
  % Options
  \setkeys{csm@figure@keys}{label=,#1}%
  \setkeys{csm@figure@keys}{caption=,#1}%
  ...
  \setkeys{csm@figure@keys}{columns=1,#1}%
  % Parse options
  \edef\csm@num@cols{\tl_use:N \l_csm_figure_cols_tl}
  \pgfmathparse{\csm@num@cols>1 ? 1:0}
  \ifthenelse{\pgfmathresult>0}{\def\csm@figarea@width{1}}{%
    \def\csm@figarea@width{\csm@singlecol@emulate@width}}
  % Make figure
  \begin{figure}
    \centering
    \begin{minipage}{\csm@figarea@width\textwidth}
    }{%
    \end{minipage}
    \caption{\tl_use:N \l_csm_figure_caption_tl}
    \figlabel{\tl_use:N \l_csm_figure_label_tl}
...
\DeclareDelayedFloatFlavor{csmfigure}{figure}
\fi

\makeatother
\ExplSyntaxOff

% Reference a subfigure that is baked into the drawing, so not really a LaTeX
% subfigure
\newcommand{\fakesubfigref}[3][]{%
  \figref{#2}(#3)\ifthenelse{\equal{#1}{}}{}{-(#1)}}
...
\newtcolorbox{tablebox}[1][]{
  ...
  boxsep=\tcb@boxsep,
  % Colors
  colback=csm@table@arxiv@bg,
  coltitle=black,
  colbacktitle=csm@table@caption@bg,
  colframe=csm@table@arxiv@border,
  % Title
...
\newenvironment{csmtable}[1][]{%
  ...
  \setkeys{csm@table@keys}{columns=1,#1}%
  % % Parse options
  \edef\csm@num@cols{\tl_use:N \l_csm_table_cols_tl}
  \pgfmathparse{\csm@num@cols>1 ? 1:0}
  \ifthenelse{\pgfmathresult>0}{\def\csm@tabarea@width{1}}{%
    \def\csm@tabarea@width{\csm@singlecol@emulate@width}}
  % Make figure
  \begin{table}[t]
    \caption{\tl_use:N \l_csm_table_caption_tl}
    \vspace*{.1in}
    \centering
    \begin{minipage}{\csm@tabarea@width\textwidth}
    }{%
    \end{minipage}
    \tablabel{\tl_use:N \l_csm_table_label_tl}
  \end{table}
}
\DeclareDelayedFloatFlavor{csmtable}{table}
\fi
...
% Keep track of how many references the sidebar bibliography has
\newcounter{sbrefs}
\renewbibmacro*{finentry}{\stepcounter{sbrefs}\finentry}

% Low-level print sidebar bibliography
\newcommand{\csm@sb@printbib}[2]{%
  \printbibliography[title=References,keyword=#1,resetnumbers=#2]}

% Print a sidebar bibliography with new references specifically for the sidebar
\newcommand{\make@sidebar@bib}{%
  \edef\sidebar@bib@label{sidebar:\tl_use:N \l_csm_sidebar_label_tl}%
  % Decision whether to reset the reference counter
  \ifmaketwocolcsm
  \def\@sb@bibnum@reset{false}%
  \else
  \pgfmathparse{\thesbrefs==0 ? 1:0}
  % \ifthenelse{\equal{\@sidebar@number}{1}}{%
  \ifthenelse{\pgfmathresult>0}{%
    \def\@sb@bibnum@reset{true}
  }{%
    \def\@sb@bibnum@reset{false}
  }%
  \fi
  ...
  \begin{refcontext}[sorting=none,labelprefix=S]%
    \ifmaketwocolcsm
    \renewcommand*{\bibfont}{\fontsize{7}{9}\sffamily}%
    \else
    ...
    \begingroup\edef\sidebar@print@bib{\endgroup\noexpand
      ...
      resetnumbers=\@sb@bibnum@reset]
    }\sidebar@print@bib
  \end{refcontext}%
}

% Prefix labels with "S" for sidebar
\newcommand{\sidebar@labels@on}{
...
\newtcolorbox{sidebarbox}[1][]{
  ...
  bottomrule=1.5pt,
  % Colors
  colback=csm@sidebar@arxiv@bg,
  colframe=black,
  colbacktitle=csm@sidebar@arxiv@bg,
  coltitle=black
}
...
\newenvironment{subsidebar}[1][noreset]{
  ...
  \begin{lrbox}{\csname\sidebar@box\endcsname}%
    \begin{minipage}{\sidebar@width}%
      \ifthenelse{\equal{\sidebar@reset}{noreset}}{}{\sidebar@labels@on}%
      % Set columns
      ...
      \ifthenelse{\equal{\sidebar@reset}{noreset}}{}{\sidebar@labels@off}%
    \end{minipage}%
  \end{lrbox}%
...
\newenvironment{sidebar}[1][]{%
  ...
  \setkeys{csm@sidebar@keys}{columns=2,#1}%
  % Parse options
  \edef\@csm@sb@title{\tl_use:N \l_csm_sidebar_title_tl}
  \edef\@csm@sb@label{sidebar:\tl_use:N \l_csm_sidebar_label_tl}%
  % New page
  \clearpage
  \newpage
  % >> Create the sidebar <<
  % Title
  \section[\@csm@sb@title]{Sidebar:~%
    \hypertarget{\@csm@sb@label}{\@csm@sb@title}}
  \sidebar@labels@on % Turn on "S" prefixes
}{
  \make@sidebar@bib
  ...
  \clearpage
  \sidebar@labels@off % Turn off "S" prefixes
}
...
% Define a queue of sidebars that are to be printed
\newcounter{sidebar@queue@size}%
\newcommand{\reset@sidebar@queue}{
  \gdef\csm@sb@queue@title{}
  \gdef\csm@sb@queue@label{}
  \gdef\csm@sb@queue@side{}
  \gdef\csm@sb@queue@position{}
  \gdef\csm@sb@queue@columns{}
  \gdef\csm@sb@queue@content{}
  \setcounter{sidebar@queue@size}{0}
}
\reset@sidebar@queue

% Push a new sidebar into the queue
\newcommand{\declaresidebar}[2][]{
  % Raw options
  \setkeys{csm@sidebar@keys}{title=,#1}%
  \setkeys{csm@sidebar@keys}{label=,#1}%
  \setkeys{csm@sidebar@keys}{side=odd,#1}%
  ...
  \setkeys{csm@sidebar@keys}{columns=2,#1}%
  % Save to queue
  \add@list@item{\csm@sb@queue@title}{\tl_use:N \l_csm_sidebar_title_tl}
  \add@list@item{\csm@sb@queue@label}{\tl_use:N \l_csm_sidebar_label_tl}
  \add@list@item{\csm@sb@queue@side}{\tl_use:N \l_csm_sidebar_side_tl}
  \add@list@item{\csm@sb@queue@position}{\tl_use:N \l_csm_sidebar_pos_tl}
  \add@list@item{\csm@sb@queue@columns}{\tl_use:N \l_csm_sidebar_cols_tl}
  \add@list@item{\csm@sb@queue@content}{#2}
  \stepcounter{sidebar@queue@size}
  % Print sidebars immediately if pretty CSM
  \ifmaketwocolcsm
  \flushsidebars
  \fi
}

% Print out all sidebars, removing them from the queue (FIFO)
\makeflag{printsb}{false}
\newcommand{\flushsidebars}{
  % Decide if there are any sidebars to be printed
  \pgfmathparse{\thesidebar@queue@size>0 ? 1:0}
  \ifthenelse{\pgfmathresult>0}{\makeprintsbtrue}{\makeprintsbfalse}
  \ifmakeprintsb
  % Reset the reference counter
  \setcounter{sbrefs}{0}
  % Make sidebar queues into indexable arrays
  \makelist{csm@sb@titles}{\csm@sb@queue@title}
  \makelist{csm@sb@labels}{\csm@sb@queue@label}
  \makelist{csm@sb@sides}{\csm@sb@queue@side}
  \makelist{csm@sb@positions}{\csm@sb@queue@position}
  \makelist{csm@sb@columns}{\csm@sb@queue@columns}
  \makelist{csm@sb@contents}{\csm@sb@queue@content}
  \foreach \@sidebar@number in {1,...,\thesidebar@queue@size} {
    \begin{sidebar}[%
      title={\list@nth{csm@sb@titles}{\@sidebar@number}},
      label={\list@nth{csm@sb@labels}{\@sidebar@number}},
      side={\list@nth{csm@sb@sides}{\@sidebar@number}},
      position={\list@nth{csm@sb@positions}{\@sidebar@number}},
      columns={\list@nth{csm@sb@columns}{\@sidebar@number}}]
      \input{\list@nth{csm@sb@contents}{\@sidebar@number}}
    \end{sidebar}
  }
  \reset@sidebar@queue % Empty the queue
  \fi
}
...
\newtcolorbox{blurbbox}{
  ...
  enhanced jigsaw,
  % Fonts
  fontupper=\sffamily\itshape\large,
  % Sizes
  width=\textwidth,
  sharp corners=all,
  boxrule=0pt,
...
\newcommand{\csmnomenclature}[2][]{
  ...
  % Write sidebar body to file
  \newwrite\file
  \immediate\openout\file=csm_\nomenc@label.tex
  \immediate\write\file{\noexpand\footnotesize}
  \immediate\write\file{\string\def\string\arraystretch{1.2}}
  \immediate\write\file{\string\newcolumntype{V}{%
      >{\string\hsize=\nomenc@width\string\linewidth}X}}
  \immediate\write\file{\string\begin{tabularx}{\string\columnwidth}{VX}}
  \immediate\write\file{  \string\input{#2}}
  \immediate\write\file{\string\end{tabularx}}
  \immediate\closeout\file
  % Push sidebar into queue
  \declaresidebar[%
  title={\nomenc@title},
  columns=1,
  label={\nomenc@label},
